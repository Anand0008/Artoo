...
class CodeProposalAgent(BaseAgent):
    def run(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            if result is None:
                # Bedrock could not parse a CodeProposal even after retries in
                # invoke_llm_structured.  Log a warning and continue the
                # pipeline without a code proposal rather than halting.
                self.logger.warning(
                    "agent_node_skipped_no_parse",
                    ticket_id=ticket_id,
                    run_id=run_id,
                    reason="LLM returned None for CodeProposal after all retries",
                )
                return {
                    "code_proposal": None,
                    ...
                    "errors": ["code_proposal: LLM returned None after all retries (skipped)"],
                }
